 ...
 *
 * @constructor
 * @param {?string=} [opt_lineSeparator='\n'] - символ перевода строки
 */
function Parser(opt_lineSeparator) {
	this.nl = opt_lineSeparator || '\n';
	this._cache = {};
	this._realpathCache = {};
}
		...
		async.waterfall([
			...
			(src, callback) => {
				fs.stat(file, (err, stat) => {
					callback(err, stat, src.replace(/\\/g, '/'));
				});
			},
...
Parser.prototype.parseFile = function (file, callback) {
	this.normalizePath(file, (err, src) => {
		...
		if (this._cache[src]) {
			return callback(null, this._cache[src], src);
		}
