...
export default class Parser {
	 ...
	 * @param {?string=} [sourceRoot] - the root for all relative URLs in the source map
	 */
	 ...
	 * @return {string}
	 */
	static relativeUrl(from, to) {
		return Parser.normalizeUrl(
			path.join(
				path.relative(from, path.dirname(to)),
				path.basename(to)
			)
		);
	}
	...
	normalizePath(file, callback) {
		file = Parser.normalizeUrl(path.resolve(file));
					...
					if (sourceMap.sourcesContent) {
						$C(sourceMap.sourcesContent).forEach((content, i) => {
							...
							$C(originalMap).forEach((el) => {
								if (el.source === src) {
									if (this.sourceRoot) {
										el.source = Parser.relativeUrl(this.sourceRoot, el.source);
									}
						...
						if (original) {
							...
							info = {
								...
								source: this.sourceRoot ?
									Parser.relativeUrl(this.sourceRoot, file) : file,
