...
export default class Parser {
	...
	parse(file, content, callback) {
		...
		content = $C(this.replacers).reduce((content, el) => {
		...
		// URL transformers
		content = content.replace(/^(([^\S\n\r]*)\/\/(?:#include|without)\s+)(.*)/gm, (sstr, decl, spaces, src) => {
			actions.push((cb) => {
				const parts = src.split('::');
				glob(path.join(dirname, parts[0]), null, (err, files) => {
					if (files) {
						const diff = files.length > 1 && this.sourceMaps ?
							`${this.nl}${spaces}//#diff ${files.length - 1}` : '';

						content = content.replace(sstr, $C(files).reduce((res, el, i) => {
							parts[0] = path.relative(dirname, el);
							res += decl + parts.join('::') + (files.length > 1 && i !== files.length - 1 ? this.nl : '');
							return res;
						}, '') + diff);
					}
