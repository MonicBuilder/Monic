...
export default class Parser {
	...
	parse(file, content, callback) {
		...
		content = content.replace(/\/\/# sourceMappingURL=(.*?)\s*$/, (sstr, url) => {
			actions.push((cb) => {
				fs.readFile(path.normalize(path.resolve(path.dirname(file), url)), (err, content) => {
					try {
						Parser.sourceMaps[file] = new SourceMapConsumer(JSON.parse(content));
		...
		async.parallel(actions, (err) => {
			...
			const
				fileStructure = new FileStructure(file, this.nl, sourceMap),
