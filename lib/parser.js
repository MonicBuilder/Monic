...
export default class Parser {
	...
	constructor(params) {
		...
		this.sourceMap = params.sourceMap || new SourceMapGenerator();
					...
					if (line.match(/^\s*\/\/#(.*)/)) {
						if (RegExp.$1) {
							...
							if (/^(include|without)$/.test(dir)) {
								return this[key](fileStructure, params, info, asyncParseCallback);

							} else if (/^(label|endlabel|if|endif|set|unset)$/.test(dir)) {
								try {
									this[key](fileStructure, params, info);
	 ...
	 */
	_include(file, value, info, callback) {
		let
	 ...
	 * @param {function(Error=)} callback - a callback function
	 */
	_without(file, value, info, callback) {
		let
			paramsParts = value.split('::'),
