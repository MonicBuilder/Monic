...
FileStructure.prototype._compileBlock = function (block, labels, flags) {
	switch (block.type) {
		...
		case 'include': {
			let key = Object.keys(block.labels).sort().join() +
				'@' +
				Object.keys(flags).sort().join();

			if (!block.fileStructure.included[key]) {
				block.fileStructure.included[key] = true;
...
FileStructure.prototype._isValidContentBlock = function (block, labels, flags) {
	switch (block.type) {
		...
		case 'label': {
			return !Object.keys(labels).length || labels[block.label];
		}
	}
