...
export class FileStructure {
	 ...
	 * @param {?number=} [opt_i] - an iteration cursor position
	 * @return {string}
	 */
	compile(opt_labels, opt_flags, opt_sourceMap, opt_i) {
		$C(opt_labels).forEach((el, key) => {
			this.root.labels[key] = true;
		});

		return this._compileBlock(this.root, this.root.labels, opt_flags || {}, opt_sourceMap, opt_i);
	}
	 ...
	 * @return {!FileStructure}
	 */
	without(opt_labels, opt_flags, opt_sourceMap, opt_i) {
		this._compileBlock(this.root, opt_labels || {}, opt_flags || {}, opt_sourceMap, opt_i);
		return this;
	}
	 ...
	 * @return {string}
	 */
	_compileBlock(block, labels, flags, opt_sourceMap, opt_i) {
		switch (block.type) {
			case 'code':
				if (!block.included) {
					block.included = true;
			...
			case 'include':
				...
				if (!this.included[cacheKey]) {
					...
					return block.fileStructure.compile(block.labels, flags, opt_sourceMap, opt_i);
				}
				...
				if (FileStructure.isValidContentBlock(block, labels, flags)) {
					...
					return $C(block.content)
						.map((block) => {
							...
							return this._compileBlock(block, labels, flags, opt_sourceMap, i);
						})
