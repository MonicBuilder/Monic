...
export class FileStructure {
	...
	getRelativePathOf(src) {
		return path.normalize(path.resolve(path.dirname(this.file), src));
	}
	 ...
	 * Sets diff
	 *
	 * @param {number} value - the diff value
	 * @return {!FileStructure}
	 */
	diff(value) {
		this.currentBlock.content.push({
	 ...
	 */
	error(msg, opt_info) {
		this.addCode(`throw new Error(${JSON.stringify(`MonicError: ${msg}`)});${this.nl}`, opt_info);
		return this;
	}
	...
	_compileBlock(block, labels, flags, opt_sourceMap) {
		switch (block.type) {
			...
			case 'diff':
				Parser.diffMap[this.uid] -= block.value;
				Parser.tmpDiff += block.value;
				break;
			...
			default:
				if (FileStructure.isValidContentBlock(block, labels, flags)) {
					return $C(block.content)
						.map((block) => {
							Parser.diffMap[this.uid] = Parser.diffMap[this.uid] || 0;

							if (!Parser.current) {
								Parser.current = this.uid;

							} else if (this.uid !== Parser.current) {
								Parser.diffMap[Parser.current]++;
								Parser.tmpDiff = Parser.diff - Parser.diffMap[this.uid];
								Parser.current = this.uid;
							}
								...
								if (!info.ignore) {
									...
									$C(selfMap ? [info] : info).forEach((info) => {
										if (selfMap) {
											...
											info.generated.line += Parser.tmpDiff;
										}
